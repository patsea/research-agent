...
function _getProfile() {
  let config;
  try {
    config = JSON.parse(readFileSync(join(__dirname_pr, '../../config/user-profile.json'), 'utf8'));
  } catch (err) {
    console.error('[portfolio-researcher] Failed to parse config file: user-profile.json', err.message);
    throw err;
  }
  return config;
}
